...
class Trends::Statuses < Trends::Base
  ...
  def eligible?(status)
    (status.public_visibility? || status.public_in_local?) && status.account.discoverable? && !status.account.silenced? && !status.account.sensitized? && !status.reply? && valid_locale?(status.language)
  end
